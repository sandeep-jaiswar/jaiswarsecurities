...
export function MarketScanner() {
  ...
  return (
    <div className="bg-terminal-panel border border-terminal-border h-48">
      ...
      <div className="p-2">
        <div className="grid grid-cols-4 gap-1 text-xs text-terminal-muted mb-2">
          <div>Symbol</div>
          <div>Change</div>
          <div>Volume</div>
          <div>Signal</div>
        ...
        <div className="space-y-1">
          {mockData.map((item, index) => (
            <div key={index} className="grid grid-cols-4 gap-1 text-xs">
              ...
              <div className={item.change.startsWith('+') ? 'text-market-up' : 'text-market-down'}>
                {item.change}
              </div>
              <div className="text-terminal-text">{item.volume}</div>
              <div className={`text-${item.signal === 'BUY' ? 'market-up' : item.signal === 'SELL' ? 'market-down' : 'terminal-muted'}`}>
                {item.signal}
              </div>
            </div>
          ))}
